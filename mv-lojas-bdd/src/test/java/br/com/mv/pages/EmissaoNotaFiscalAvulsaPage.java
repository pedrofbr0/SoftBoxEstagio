...
public class EmissaoNotaFiscalAvulsaPage extends PageObject {
	...
	public void selecionarProdutoNotaFiscal(String produto) {
		$(".//*[@id='tabelaCamposLinha:tContent']/tbody/tr/td[text()='" + produto + "']/parent::tr/td[1]/input").click();
	}
	...
	public void salvarNroNotaFiscalSessao() {
		String[] msg = mensagem.getMsg().split(":");
		
		String nroNotaFiscal = msg[1].trim();
		
		System.out.println("NroNota: " + nroNotaFiscal);
		Serenity.setSessionVariable(NRO_NOTA_FISCAL).to(nroNotaFiscal);
	}
	
	public void clickBtnConcluir() throws InterruptedException {
		this.btnConcluir.click();
		
		// Deseja confirmar a emissão desta Nota Fiscal?
		new WebDriverWait(this.getDriver(), 40).until(ExpectedConditions.alertIsPresent());
		controleJanela.abrirPopupAlert(true);
		
		Thread.sleep(2000);
		
		salvarNroNotaFiscalSessao();
		
		Assert.assertTrue("A Nota fiscal nao foi gerada", mensagem.getMsg().contains("Notas Fiscais Eletrônicas geradas com sucesso"));
	}
	...
	public void setNroNotaSessao(String tipoNFAvulsa) {
		...
		if (tipoNFAvulsa.equals("51") || tipoNFAvulsa.equals("32")) {
			nroNFAvulsa = Integer.toString(nfDAO.getNotaFiscalSemPedido(nroLoja, tipoNFAvulsa, nroNF).getNroNf());
		} else {
			nroNFAvulsa = Integer.toString(nfDAO.getNotaFiscal(nroLoja, nroPedido, tipoNFAvulsa).getNroNf());
		}
	...
	public void concluirEmissaoNFAvulsa(String tipoNFAvulsa) throws InterruptedException {
		this.clickAbaItens().setObsItemNF("Teste");
		this.clickBtnConcluir();
		...
		//Thread.sleep(2000);
		//mensagem.clickBtnFechar();
		//controleJanela.fecharJanela(mensagem.getBtnFechar());
		
		/*new WebDriverWait(this.getDriver(), 40).until(ExpectedConditions.alertIsPresent());
		// Deseja realizar impressão NFE?
		controleJanela.voltarJanelaPrincipal(false);*/
		
		this.setNroNotaSessao(tipoNFAvulsa);
		
	}
	
	public void salvarNotaFiscalRelatorio(String codTeste) throws IOException {
		Boolean gravarNotaFiscalRelatorio = Boolean.parseBoolean(SystemEnvironmentVariables.createEnvironmentVariables().getProperty("mvlojas.gravar.nota_fiscal.relatorio"));
		
		if (gravarNotaFiscalRelatorio) {
			String nroNotaFiscal = (String) Serenity.sessionVariableCalled(NRO_NOTA_FISCAL);
			
			if (nroNotaFiscal.isEmpty()) { // Se nao tiver nota na sessao, tenta pegar da tela de consulta
				nroNotaFiscal = $(".//*[@id='table:tContent']/tbody/tr[1]/td[3]").getText();
			}
			
			RelatorioAutomacao.salvarNotaFiscalRelatorio(codTeste, nroNotaFiscal);
		}
	}
	
	public void salvarCupomFiscalRelatorio(String codTeste) throws IOException {
		Boolean gravarNotaFiscalRelatorio = Boolean.parseBoolean(SystemEnvironmentVariables.createEnvironmentVariables().getProperty("mvlojas.gravar.nota_fiscal.relatorio"));
		
		if (gravarNotaFiscalRelatorio) {
			String nroNotaFiscal = $(".//*[@id='table:tContent']/tbody/tr/td[10]").getText();
	...
	public void clicarAbaNotaFiscalAvulsa(String aba) {
		$(".//*[@class='tabbedPane']/tbody/tr[1]/td/table/tbody/tr/td[@class='tab']/span[contains(text(), '" + aba + "')]").click();
	}
	
	public void clicarBotaoIncluirProduto() throws InterruptedException {
		controleJanela.abrirPopupClick(this.getBtnIncluirProduto());
	}
	
	public void informarQuantidadeProdutoNotaFiscal(String quantidade) {
		$(".//*[@name='quantidadeItemNotaFiscal']").sendKeys(quantidade);
	}
	
	public void informarAliquotaICMSNotaFiscal(String aliquotaICMS) {
		$(".//*[@name='aliquotaIcmsItemNotaFiscal']").sendKeys(aliquotaICMS);
	}
	
	public void informarBaseCalculoICMSNotaFiscal(String baseCalculoICMS) {
		$(".//*[@name='valorBaseIcmsItemNotaFiscal']").sendKeys(baseCalculoICMS);
	}
	
	public void informarValorICMSNotaFiscal(String valorICMS) {
		$(".//*[@name='valorIcmsItemNotaFiscal']").sendKeys(valorICMS);
	}
	
	public void informarValorCFOPNotaFiscal(String valorCFOP) {
		$(".//*[@name='cfop']").clear();
		$(".//*[@name='cfop']").sendKeys(valorCFOP);
	}
	
	public void informarQuantidadeTodosProdutosNotaFiscal(String qtdProduto) {
		
		  List<WebElement> txtsQuantidadeProdutos = this.getDriver().findElements(By.cssSelector("input[id^=txtQuantidade]"));
		  
		  for (WebElement txtQtdProduto: txtsQuantidadeProdutos) {
			  txtQtdProduto.sendKeys(qtdProduto);
		  }
	}
	
	public void informarDescricaoProdutoGeral(String descricao) {
		$(".//*[@id='txtDescricaoProduto0_id99999']").clear();
		$(".//*[@id='txtDescricaoProduto0_id99999']").sendKeys(descricao);
	}
	
	public void informarValorProdutoGeral(String vlrItem) {
		$(".//*[@id='txtValorUnitarioProduto0_id99999']").sendKeys(vlrItem);
	}
	
	public void informarAliquotaICMS(String aliquotaICMS) {
		$(".//*[@id='txtAliquotaICMSProduto0_id99999']").sendKeys(aliquotaICMS);
	}
	
	public void informarBaseCalculoICMS(String baseCalculoICMS) {
		$(".//*[@id='txtValorBaseIcmsItemNotaFiscal0_id99999']").sendKeys(baseCalculoICMS);
	}
	
	public void informarValorICMS(String valorICMS) {
		$(".//*[@id='txtValorIcmsItemNotaFiscal0_id99999']").sendKeys(valorICMS);
	}
	
	public void aprovarNotasPedido() {
		int codNroLoja = Integer.parseInt(Serenity.sessionVariableCalled(COD_NRO_LOJA));
		int nroPedido = Integer.parseInt(Serenity.sessionVariableCalled(NRO_PEDIDO));
		
		nfDAO.aprovarNotasPedido(codNroLoja, nroPedido);
	}
}