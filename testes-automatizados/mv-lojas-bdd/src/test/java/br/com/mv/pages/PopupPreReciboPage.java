...
import static br.com.mv.model.SessionVariables.VLR_PEDIDO_TROCA_COM_REVERSAO;
import static br.com.mv.model.SessionVariables.VLR_PEDIDO_TROCA_SEM_REVERSAO;
...
public class PopupPreReciboPage extends PageObject {
	...
	@FindBy(xpath = ".//*[@id='btnAprovar']")
	private WebElement btnAprovarPlanilha;
	...
	public void registrarValoresPedidoComSemReversao() {
		String msgReversaoCredito = this.getDriver().switchTo().alert().getText();
		
		String[] partesMsg = msgReversaoCredito.split(":");
		
		for (String str : partesMsg) {
			System.out.println("->" + str);
		}
		
		String[] str2 = partesMsg[2].split(" ");
		
		for (String str1 : str2) {
			System.out.println(">" + str1);
		}
		
		Double vlrComReversao = Double.parseDouble(partesMsg[2].split(" ")[1].trim()); // devolve um juros pagos
		Double vlrSemReversao = Double.parseDouble(partesMsg[3].trim()); // nao devolve os juros pagos
		
		Serenity.setSessionVariable(VLR_PEDIDO_TROCA_COM_REVERSAO).to(vlrComReversao);
		Serenity.setSessionVariable(VLR_PEDIDO_TROCA_SEM_REVERSAO).to(vlrSemReversao);
	}
	
	public void validarValorPedidoComReversao() {
		
		double vlrPedidoComReversaoTela = Double.parseDouble($(".//*[@id='txtValorTotalPedido']").getValue().replace(",", "."));
		double vlrPedidoComReversaoSessao = Serenity.sessionVariableCalled(VLR_PEDIDO_TROCA_COM_REVERSAO);
		
		System.out.println(vlrPedidoComReversaoTela+" : "+ vlrPedidoComReversaoSessao);
		
		Assert.assertTrue("Os valores de reversão do pedido estao incorretos.", vlrPedidoComReversaoTela == vlrPedidoComReversaoSessao);
	}
	
	public void validarValorPedidoSemReversao() {
		double vlrPedidoSemReversaoTela = Double.parseDouble($(".//*[@id='txtValorPrPe']").getValue().replace(",", "."));
		double vlrPedidoSemReversaoSessao = Serenity.sessionVariableCalled(VLR_PEDIDO_TROCA_SEM_REVERSAO);
		
		System.out.println(vlrPedidoSemReversaoTela+" : "+ vlrPedidoSemReversaoSessao);
		
		Assert.assertTrue("Os valores de reversão do pedido estao incorretos.", vlrPedidoSemReversaoTela == vlrPedidoSemReversaoSessao);
	}
	
	public void selecionarTrocaComReversao() throws InterruptedException {
		new WebDriverWait(this.getDriver(), 40).until(ExpectedConditions.alertIsPresent());
		Thread.sleep(2000);
		
		this.registrarValoresPedidoComSemReversao();
		
		this.getDriver().switchTo().alert().accept();
	}
	
	public void selecionarTrocaSemReversao() throws InterruptedException {
		new WebDriverWait(this.getDriver(), 40).until(ExpectedConditions.alertIsPresent());
		Thread.sleep(2000);
	...
	public WebElement getBtnAprovarPlanilha() {
		return this.btnAprovarPlanilha;
	}
	...
	public void informarNumeroPedido() {
		...
		String numPedido = "4934310"; //(String) Serenity.sessionVariableCalled(NRO_PEDIDO); //"4932142"; //
	...
	public void informarFormaPagtoDevolucao(String formaPagto) {
		$(".//*[@id='cmbFormaPagtoDev0']").selectByVisibleText(formaPagto);
	}
	
	public void informarVlrPagtoDevolucao() {
		String vlrDevolucao = $(".//*[@id='txtTotalDevolucaoPreRecibo']").getValue();
		
		$(".//*[@id='txtVlrDev0']").clear();
		$(".//*[@id='txtVlrDev0']").sendKeys(vlrDevolucao);
	}
	
	public void clicarBtnAprovarPlanilhas() throws InterruptedException {
		//controleJanela.fecharJanela(this.getBtnAprovarPlanilha());
		this.btnAprovarPlanilha.click();
