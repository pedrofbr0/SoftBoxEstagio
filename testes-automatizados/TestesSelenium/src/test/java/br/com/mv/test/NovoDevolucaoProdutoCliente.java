 ...
 *
 */
public class NovoDevolucaoProdutoCliente {
	...
	private static final String DESC_TIPO_NF_DEVOLUCAO_VENDA_MAIS_NOVENTA_DIAS_LEGADO = "323 - DEVOLUÇÃO DE VENDAS (COM MAIS DE 90 DIAS)-SIST LEGADO IN UF-1275";
	
	private static final int NRO_PRE_RECIBO = 2;
	
	private static final String BANDEIRA = "SALFER";
	private static int NRO_PEDIDO = 24756685; //1153192; //1153031; //1153030;
	private static int NRO_LOJA_PEDIDO = 24; // Filial pedido
	...
	public void teste1() throws InterruptedException, SQLException, IOException {
		...
		// PROCESSO ANTIGO (SUBSTITUIDO POR UMA TELA)
		/*Thread.sleep(1000);
		...
		// ENTRAR NO MENU: APROVAÇÃO FINANCEIRA
		controleMenu.navegarMenuNovo("Troca/Devolução", "Aprovação Financeira");
		agCarregamento.aguardarCarregamentoNovaArquitetura2();
		
		planilhaAprovacao.setFiltroNroPreRecibo(Integer.toString(nroLojaPreReciboReverso)).clickRadioPedidoLegado().clickBtnPesquisar();
		planilhaAprovacao.clickEditarAprovacaoPlanilha(nroLojaPreReciboReverso);
		planilhaAprovacao.configuracaoDevolucao();
		
		// VOLTAR PARA O MENU: TROCA/DEVOLUÇÃO > PRÉ-RECIBOS
		controleMenu.navegarMenuNovo("Troca/Devolução", "Pré-Recibos");
		...
		controleFatCarga.clickAssumirDevolucao();
		// Devolução assumida com sucesso
	...
	public void teste3GerarCredito() throws InterruptedException {
		...
		pesquisaPreRecibo.setFiltroNroPreRecibo(Integer.toString(nroPreReciboReverso));
		agCarregamento.aguardarCarregamentoNovaArquitetura2();
		pesquisaPreRecibo.clickPesquisar();
