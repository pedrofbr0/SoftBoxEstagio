...
public class TrocaUmServico {
	...
	private static final int NRO_PEDIDO = 55212;
	...
	private static final String TIPO_FLUXO_APROVACAO_DESC = "Troca"; // 1 - Troca
	
	private static final int STATUS_AG_FLUXO_APROVACAO = 2; // AG. APROVAÇÃO DE SERVIÇOS
	...
	public void testeTrocaDevolucao() throws InterruptedException, SQLException {
		...
		preRecibo.setNroPedido(Integer.toString(NRO_PEDIDO)).setNroEmpresa(DESC_EMPRESA).setCodNroLoja(Integer.toString(COD_NRO_LOJA))
							.setProcesso(TIPO_FLUXO_APROVACAO_DESC).clickPesquisar();
		agCarregamento.aguardarCarregamentoArquiteturaAntiga();
		...
		cadSolicitacao.informarMotivoServico(servicos, "3 - SERVICO NAO UTILIZADO", "Trocando serviço teste...");
		...
		int nroPreRecibo = new PreReciboDAO().getPreRecibo(NRO_LOJA, NRO_PEDIDO, STATUS_AG_FLUXO_APROVACAO);
	...
	private void removerProdutos(List<Produto> produtos) {
		...
		for (Produto prod : produtos) {
			preRecibo.selecionarProduto(prod.getNroProduto()).clickRemoverProduto();
		}
		
	}
	
	/**
	 * Deixa apenas um serviço que necessite de autorização no pedido
	...
	private void removerServicos(List<Servico> servicos) {
		...
		for (int i = 1; i < qtdServicos; i++) {
			preRecibo.selecionarServico(servicos.get(i).getNroServico()).clickRemoverServico();
		}
	}
