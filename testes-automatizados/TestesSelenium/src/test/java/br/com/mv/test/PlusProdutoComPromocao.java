...
public class PlusProdutoComPromocao {
	...
	private static final String IP = "http://10.30.237.21:8080/"; 
	...
	public void test() throws InterruptedException, SQLException {
		...
		System.out.println("PLUS - PRODUTO COM PROMOÇÃO");
		...
		produto.put("ind_sel_promocao", "true");
		...
		System.out.println("Comissao plus prod banco: " + Float.toString(plus.getPlusProdComPromocao()));
		...
		Assert.assertEquals("O valor de plus do produto com promoção está divergente.", Float.toString(plus.getPlusProdComPromocao()), Double.toString(comissaoPlusProd));
		Assert.assertEquals("O valor de plus do juros com promoção está divergente.", Float.toString(plus.getPlusJurosComPromocao()), Double.toString(comissaoPlusJuros));
