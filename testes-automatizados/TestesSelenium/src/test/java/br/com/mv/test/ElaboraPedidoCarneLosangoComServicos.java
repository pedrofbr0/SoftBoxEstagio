...
public class ElaboraPedidoCarneLosangoComServicos {
	...
	private static final String DATA_NASCIMENTO = "01/05/1980";
	...
	public void test() throws InterruptedException {
		...
		///load.aguardarCarregamentoNovaArquitetura();
		load.aguardarCarregamentoArquiteturaAguarde();
		...
		tipoVenda = "66 - CARNE LOSANGO SCRED";
		String plano = "8 X";
	...
	public void consultaPreSimplificada() throws InterruptedException {
		...
		System.out.println("CPF: " + consultaPreSimplificada.getCpf());
		
		load.aguardarCarregamentoNovaArquitetura();
		
		if (consultaPreSimplificada.getCpf() == "") {
			
			load.aguardarCarregamentoArquiteturaAguarde();
			consultaPreSimplificada.setTipoVenda(tipoVenda).setDataNascimento(DATA_NASCIMENTO + Keys.ENTER).setCpf(CPF);
		...
		if (!msgAvaliacao.equals("Elegível") && msgConsulta.equals("CONSULTA SIMPLIFICADA EM ANDAMENTO, AGUARDE ALGUNS INSTANTES E ACIONE CONSULTAR")){
			...
			for (int i = 0; i <= 120; i++) {
				...
				if (i == 120){
					...
					consultaPreSimplificada.setTipoVenda(tipoVenda).clickConsultar();
					...
					System.out.println("Mensagem de avaliação após aguardar: " + msgAvaliacao);
