...
import br.com.mv.dao.cargaVeiculo.Carga;
import br.com.mv.dao.cargaVeiculo.CargaDAO;
import br.com.mv.jdbc.Conexao;

/**
 * NotaFiscalDAO - Class crud Notafiscal
 * 
...
public class NotaFiscalDAO {
	 ...
	 * @return
	 */
	public List<NotaFiscal> getNotaFiscal(int nroLoja, int nroPedido, String tiposNf) {
		...
		try {
			...
			while (rs.next()) {
				...
				nfs.setNroTipoNf(rs.getInt("NRO_TIPO_NF"));
				nfs.setNroCupom(rs.getInt("NRO_CUPOM"));
				nfs.setCodTipoDocto(rs.getString("COD_TIPO_DOCTO"));
				nfs.setNroNf(rs.getInt("NRO_NF"));
				nfs.setSerieNf(rs.getString("SERIE_NF"));
				nfs.setNroLoja(rs.getInt("NRO_LOJA"));
				nfs.setCodNroLoja(rs.getInt("COD_NRO_LOJA"));
				nfs.setNroLojaDestino(rs.getInt("NRO_LOJA_DESTINO"));
