...
var dadosPedido = require('../../data/025/pedido_venda_produto.json');
...
/*
 * CASOS DE TESTE - Documentação test 025
 */

/*@estrutura da suite de teste*/
casper.test.begin('\n@TestCase:\n PEDIDO COM TIPO DE VENDA CARNE LOSANGO', 0, function suite(test) {
...
function execTest(data) {
    ...
    casper.then(function () {
        telaProduto.produtos(dadosPedido[data.banco].reabertura1.produtos, true, null, 2);
    });
    ...
    casper.then(function () {
        telaProduto.produtos(dadosPedido[data.banco].reabertura2.produtos);
    });

    casper.then(function () {
        mvLojas.$valCombo("#frm-pedido #cbx-plano", dadosPedido[data.banco].reabertura2.plano);
    });

    casper.then(function () {
        mvLojas.$val("#frm-pedido #txt-entrada", dadosPedido[data.banco].reabertura2.valor_entrada);
        utils.takeSS("vlrEntrada");
    });

    casper.then(function () {
        telaGarantias.garantias(dadosPedido[data.banco].reabertura2.garantias, data);
    });
    ...
    casper.then(function () {
    	telaLiberacoes.liberacoes(dadosPedido[data.banco].reabertura2.liberacoes);
    });
