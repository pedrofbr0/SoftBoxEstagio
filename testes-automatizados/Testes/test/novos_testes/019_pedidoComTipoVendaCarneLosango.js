...
var dadosPedido = require('../../data/019/pedido_venda_produto.json');
...
/*
 * CASOS DE TESTE - Documentação test 019
 */
...
function execTest(data) {
    ...
    casper.then(function () {
    	if (data.ind_log) {
    		mvLojas.logTexto("-------------  ELABORAÇÃO DO PEDIDO -------------\n");
        }
    });
    ...
    casper.then(function () {
    	if (data.ind_log) {
	    	mvLojas.gravarLog("019_pedidoComTipoVendaCarneLosango");
    	}
    });
    ...
    casper.then(function () {
        mvLojas.click("#tab-itens-pedido a[id='tab-produtos']");
        utils.takeSS("abaProdutos");
    });
